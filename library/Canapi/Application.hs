module Canapi.Application where
...
corsify = WaiCors.cors (const (Just policy))
  where
...
whenNoSegmentsIsLeft application request =
  if RequestAccessor.hasNoSegmentsLeft request
...
routingTree (RoutingTree.RoutingTree segmentParser methodHandlerMap) = refineSegmentOr onNoSegment onSegment
  where
    onSegment =
      segmentParser >>> \case
